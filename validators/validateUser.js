...
const getRangeLengthErr = (min, max) =>
...
const validateUser = [
  body("username")
    .trim()
    .notEmpty()
    .withMessage(`User name ${emptyErr}`)
    .isAlphanumeric("en-US")
    .withMessage(`User name ${alphaNumErr}`)
    .isLength({ min: 1, max: 30 })
    .withMessage(`User name ${getRangeLengthErr(1, 30)}`)
  ...
  body("displayname")
    ...
    .isAlphanumeric("en-US")
    .withMessage(`Display name ${alphaNumErr}`)
    .isLength({ min: 1, max: 30 })
    .withMessage(`Display name ${getRangeLengthErr(1, 30)}`),
  body("password")
    .isLength({ min: 6 })
    .withMessage(`Password ${getMinLengthErr(6)}`)
    .isLength({ max: 50 })
    .withMessage(`Password ${getMaxLengthErr(50)}`)
    .custom((password, { req }) => {
